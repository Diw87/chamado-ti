...
const USERNAME_DOMAIN = "chamado-ti.local";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: caller, error: callerError } = await adminClient.from("profiles").select("role,active").eq("id", user.id).single();
    ...
    if (username.length < 3 || username.length > 40) return json({ error: "Usuário inválido." }, 400);
    ...
    const { error: profileError } = await adminClient.from("profiles").update({ username, full_name: fullName, role, department_id: departmentId, active: true }).eq("id", created.user.id);
    if (profileError) {
